...
//console.debug( "Preload: " + nextAudio.id );
		...
		if ( audioFile != null ) {
			...
			try {
                           var audioSource = document.createElement( 'source' );
                           // audiodict is the dictionary with the encoded audio generated by our CMake target "audio"
                           audioSource.src = audiodict['audio/'+prefix+indices+suffix] ;
		       	   audioElement.insertBefore(audioSource, audioElement.firstChild);
			   audioExists = true;
                           if (false)  { // old code making html request. Replaced by above one that uses encoded files
